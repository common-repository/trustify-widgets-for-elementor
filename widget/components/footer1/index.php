...
function trfy_footerHtml($settings, $profile_data) {
    $selected_language = $settings['lang_options'];

    // Languages
    if ($selected_language === 'lang_auto') {
        $selected_language = determine_locale(); // Auto detect language
    ...
    // Get data from API
    $average = isset($profile_data['average']) ? $profile_data['average'] : '0'; // Check that it's not null and provide a default
    $total_reviews = isset($profile_data['total']) ? $profile_data['total'] : '0'; // Check that it's not null and provide a default
    $profile_name = isset($profile_data['profileName']) ? $profile_data['profileName'] : 'Unknown Profile'; // Correctly reference profileName

    // JSON-LD for Google Rich Snippet
    ?>
    <script type="application/ld+json">
    {
        "@context": "https://schema.org",
        "@type": "Organization",
        "name": "<?php echo esc_html($profile_name); ?>",
        "url": "https://in.trustify.ch/<?php echo esc_html($settings['api_profile_username']); ?>", // URL of the review profile
        "aggregateRating": {
            "@type": "AggregateRating",
            "ratingValue": "<?php echo esc_html($average); ?>",
            "reviewCount": "<?php echo esc_html($total_reviews); ?>",
            "bestRating": "5",  // Highest possible rating
            "worstRating": "1"   // Lowest possible rating
        }
    }
    </script>
    
    <div class="w-grid_wrapper">
      <div class="rating-title">
        <div id="w-title">
          ...
          <?php esc_html_e('on', 'trustify-widgets-for-elementor'); ?></span>
