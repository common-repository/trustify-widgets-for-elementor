...
function trfy_gridHtml($settings, $profile_data, $feedback_data){
  ...
  ?>
    ...
    <?php esc_html_e('on', 'trustify-widgets-for-elementor'); ?></span>
      ...
      <div class="grid-row">
      <?php
      $totalFeedback = count($feedback_data["items"]);
      $column1Limit = ceil($totalFeedback / 3);
      $column2Limit = ceil(($totalFeedback - $column1Limit) / 2) + $column1Limit;
      $column3Limit = $totalFeedback;
      
      $column1Count = $column2Count = $column3Count = 0;
      
      foreach ($feedback_data["items"] as $key => $feedback) {
          if ($column1Count < $column1Limit) {
              // Print in the first column
              if ($column1Count == 0) {
                  echo '<div class="grid-column">';
              }
              trfy_renderGridChild($feedback, $settings); // Your grid-child content
              $column1Count++;
              if ($column1Count == $column1Limit) {
                  echo '</div>';
              }
          } elseif ($column2Count < $column2Limit - $column1Limit) {
              ...
              trfy_renderGridChild($feedback, $settings); // Your grid-child content
              $column2Count++;
              if ($column2Count == $column2Limit - $column1Limit) {
                  echo '</div>';
              }
          } else {
              // Print in the third column
              if ($column3Count == 0) {
                  echo '<div class="grid-column">';
              }
              trfy_renderGridChild($feedback, $settings); // Your grid-child content
              $column3Count++;
              if ($column3Count == $column3Limit - $column2Limit) {
                  echo '</div>';
              }
          }
      }
...
function trfy_renderGridChild($feedback, $settings) {
  ?>
